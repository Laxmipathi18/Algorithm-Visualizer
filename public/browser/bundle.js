...
Board.prototype.changeStartNodeImages = function() {
  ...
  if (unweighted.includes(this.currentAlgorithm)) {
    ...
    document.getElementById("weightLegend").className = "l";
    for (let i = 0; i < 14; i++) {
      let j = i.toString();
      let backgroundImage = document.styleSheets["1"].rules[j].style.backgroundImage;
...
Board.prototype.toggleTutorialButtons = function() {
  ...
  function nextPreviousClick() {
    if (counter === 1) {
      ...
      document.getElementById("tutorial").innerHTML = `<h3>Meet the algorithms</h3><h6>Not all algorithms are created equal.</h6><ul><li><b>Dijkstra's Algorithm :</b> the father of pathfinding algorithms; guarantees the shortest path</li><li><b>A* Search :</b> arguably the best pathfinding algorithm; uses heuristics to guarantee the shortest path much faster than Dijkstra's Algorithm</li><li><b>Greedy Best-first Search :</b> a faster, more heuristic-heavy version of A*; does not guarantee the shortest path</li><li><b>Breath-first Search :</b> a great algorithm; guarantees the shortest path</li><li><b>Depth-first Search :</b> a very bad algorithm for pathfinding; does not guarantee the shortest path</li></ul><div id="tutorialCounter">${counter}/8</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`
    } else if (counter === 5) {
